...
Following leakage safe pipeline:
...
5. Train Gradient Boosting Regressor model
6. Evaluate performance on both sets
7. Save model, feature importance, and metrics
"""
...
def prepare_modeling_data(df_features):
    ...
    exclude_cols = [
        ...
        # cause we dropped it in the feature engineering functions
...
def train_gb_model(X_train, y_train):
    ...
    print("Hyperparameters: n_estimators=300, learning_rate=0.1, max_depth=3, random_state=42")
...
def evaluate_model(model, X_train, y_train, X_test, y_test, model_name="Gradient Boosting Regressor"):
...
def save_model_and_data(model, feature_names, X_train, X_test, y_train, y_test, metrics, y_train_pred, y_test_pred):
    """Save model, feature names, processed data, and metrics"""
    base_dir = Path(__file__).resolve().parent.parent
    ...
    # Save model
    model_path = base_dir / 'models' / 'gradient_boosting_regressor.joblib'
    joblib.dump(model, model_path)
    print(f"Model saved to: {model_path}")

    # Save feature names
    feature_path = base_dir / 'models' / 'gb_feature_names.joblib'
    ...
    feat_imp_path = base_dir / 'results' / 'gb_feature_importances.csv'
    ...
    train_path = base_dir / 'data' / 'processed' / 'train_set_gb.csv'
    test_path = base_dir / 'data' / 'processed' / 'test_set_gb.csv'
    ...
    # Save metrics
    metrics_path = base_dir / 'results' / 'gb_metrics.txt'
    with open(metrics_path, 'w', encoding='utf-8') as f:
        f.write(f"Gradient Boosting Regressor Performance Metrics\n")
        f.write(f"=================================\n\n")
    ...
    return model_path, feature_path, feat_imp_path, train_path, test_path, metrics_path

def main():
    """Main training pipeline"""
    print("=" * 70)
    print(" Implementing leakage-safe feature engineering and Gradient Boosting modeling")
    print("=" * 70)
    print()
    ...
    model = train_gb_model(X_train_model, y_train)

    # Evaluation
    metrics, y_train_pred, y_test_pred = evaluate_model(model, X_train_model, y_train, X_test_model, y_test)

    # Save model, data, and metrics
    save_model_and_data(model, feature_names, X_train_model, X_test_model, y_train, y_test, metrics, y_train_pred, y_test_pred)
