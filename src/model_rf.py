...
def train_rf_model(X_train, y_train):
    ...
    print("Hyperparameters: n_estimators=300, max_depth=20, max_features='sqrt', min_samples_leaf=5, random_state=42")
...
def evaluate_model(model, X_train, y_train, X_test, y_test, model_name="Random Forest Regressor"):
...
def save_model_and_data(model, feature_names, X_train, X_test, y_train, y_test, metrics, y_train_pred, y_test_pred):
    ...
    feat_imp_path = base_dir / 'results' / 'rf_feature_importances.csv'
    ...
    train_path = base_dir / 'data' / 'processed' / 'train_set_rf.csv'
    ...
    # Save metrics
    metrics_path = base_dir / 'results' / 'rf_metrics.txt'
    with open(metrics_path, 'w', encoding='utf-8') as f:
        f.write(f"Random Forest Regressor Performance Metrics\n")
        f.write(f"===============================\n\n")
        f.write(f"Hyperparameters:\n")
        f.write(f"  n_estimators: 300\n")
        f.write(f"  max_depth: 20\n")
...
def main():
    ...
    print("=" * 70)
    print(" Implementing leakage-safe feature engineering and Random Forest modeling")
    print("=" * 70)
    print()
    ...
    model = train_rf_model(X_train_model, y_train)
